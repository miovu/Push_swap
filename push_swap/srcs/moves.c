...
/*   Updated: 2025/02/16 20:34:27 by chillhoneyy      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_a(t_stack *from, t_stack *to)
{
	...
	{
		if (temp->index == from->head->next->index)
			swap_nodes(from, 'a', 1);
	...
	{
		if (temp->target->index == to->head->next->index)
			swap_nodes(from, 'b', 1);
		else if (temp->target->index < (to->size / 2))
			rotate_nodes(to, 'b', 1);
		else if (temp->target->index > (to->size / 2))
...
// 			rotate_both(from, to);
// 	}
// 	else if (temp->index == from->head->next->index
// 			&& temp->target->index == to->head->next->index)
// 		swap_both(from, to);
// 	else if ((temp->index >= (from->size / 2))
// 		&& (temp->target->index >= (to->size / 2)))
// 	{
